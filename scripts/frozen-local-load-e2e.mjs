...
(async () => {
  ...
  // Re-aimed 2026-09-15: the menu now lists EVERY downloaded model (parity with original Hermes).
  ok("nothing loaded: picker lists the downloaded models, including the remembered one", !!row0 && row0.models.includes(H.id) && row0.models.includes(G.id), row0 && JSON.stringify(row0.models));
  ...
  // Re-aimed 2026-09-15: an unconfirmed pick over a loaded model now asks for a Confirm instead of refusing.
  const confirmPick = await pick(c, sid2, H.id).then((r) => r, (e) => e);
  ok("other model loaded: selecting a different model asks for a confirm (nothing evicted)", !!confirmPick && confirmPick.confirm_required === true && mock.loads.length === loadsBefore5, JSON.stringify(confirmPick));
  const row5 = await frozenRow(c, null);
  ok("other model loaded: the loaded model is listed first", !!row5 && row5.models[0] === G.id, row5 && JSON.stringify(row5.models));
